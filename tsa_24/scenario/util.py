...
def cbm_report(fm, cbm_output, biomass_pools, dom_pools, fluxes, gross_growth, product_flux):
    ...
    # annual_stock_change = pd.DataFrame({
    #     'Year': annual_carbon_stocks['Year'],
    #     'Stock_Change': annual_carbon_stocks['Total_Ecosystem'].diff().fillna(0)})
...
#                                          'Products':pi['Products'].sum()})
    ...
    annual_product_flux = pd.DataFrame({'Year':fi['timestep'],
                                        'Product_flux':fi[product_flux].sum(axis=1)})
    ...
    annual_product_stock.groupby('Year').sum().plot(
        figsize=(5,5), xlim=(0, n_steps), ylim=(None, None),
        title="Annual Product Stock", xlabel="Year", ylabel="tons of C"
    )

    annual_product_flux.groupby('Year').sum().plot(
        figsize=(5,5), xlim=(0, n_steps), ylim=(None, None),
        title="Annual Product Flux", xlabel="Year", ylabel="tons of C"
    ) 
#     # print(annual_carbon_stocks.groupby('Year').sum())
#     # print(annual_all_emissions.groupby('Year').sum())
#     # print(annual_stock_change.groupby('Year').sum())
#     # print(annual_gross_growth.groupby('Year').sum())
    
    
    # years = list(range(0, 201, 10))  # Generates a list [0, 10, 20, ... , 200]
    # df_cs = annual_carbon_stock.groupby('Year').sum().loc[years]
    # df_ae = annual_all_emission.groupby('Year').sum().loc[years]
    # df_gg = annual_gross_growth.groupby('Year').sum().loc[years]
    # df_ps = annual_product_stock.groupby('Year').sum().loc[years]
    ...
    df_ps = annual_product_stock.groupby('Year').sum()
    df_pf = annual_product_flux.groupby('Year').sum()
    ...
    merged_df = pd.merge(pd.merge(pd.merge(pd.merge(df_cs, df_ae, left_index=True, right_index=True, how='outer'),
                         ...
                         df_ps, left_index=True, right_index=True, how='outer'),
                         df_pf, left_index=True, right_index=True, how='outer')

    
    merged_df['Stock_Change']=merged_df['Ecosystem'].diff()*(-1)
    merged_df.at[0,'Stock_Change'] = 0.
    merged_df['Emissions - Growth'] = merged_df['All_Emissions'] - merged_df['Gross_Growth']
    merged_df['Stock Change - Product_flux'] = merged_df['Stock_Change'] - merged_df['Product_flux']
    
    # years = list(range(0, 201, 10))  # Generates a list [0, 10, 20, ... , 200]
    # df_period = merged_df.loc[years]
    # df_period = df_period.apply(lambda x: x.round(3) if x.dtype.kind in 'fc' else x)
    
    # df_ne=merged_df[['All_Emissions', 'Gross_Growth', 'Emissions - Growth']]
    # df_e=df_e.iloc[1:] 
    # summed_decades_df = df_ne.groupby((df_ne.index // 10) * 10).sum()
    
#     merged_df['Stock_Change']=merged_df['Ecosystem'].diff()
#     merged_df.at[0,'Stock_Change'] = 0.
#     merged_df['Growth - Emissions'] = merged_df['Gross_Growth'] - merged_df['All_Emissions']
#     merged_df['Stock Change + Product'] = merged_df['Stock_Change'] + merged_df['Product']
    
    # # Combine all the results into a single DataFrame
    # df = annual_carbon_stock.merge(df_cs, on='Year')\
    #                         .merge(df_ae, on='Year')\
    #                         .merge(df_gg , on='Year')\
    #                         .merge(df_ps, on='Year')
...
def compare_ws3_cbm(fm, cbm_output, disturbance_type_mapping, biomass_pools, dom_pools, fluxes, gross_growth, product_flux, cbm_x_shift=False):
    ...
    if cbm_x_shift:
        df_cbm = pd.DataFrame({'period':pi["timestep"] * 0.1, 
                               ...
                               'total_emission':fi[fluxes].sum(axis=1),
                               'gross_growth':fi[gross_growth].sum(axis=1),
                               'product_flux':fi[product_flux].sum(axis=1)}).groupby('period').sum().iloc[0::10, :].reset_index()
        # df_cbm.at[0,'biomass_stock']=pi[biomass_pools].sum(axis=1).iloc[0]
        # df_cbm.at[0,'dom_stock']=pi[dom_pools].sum(axis=1).iloc[0]
        # df_cbm.at[0,'eco_stock']=pi[eco_pools].sum(axis=1).iloc[0]
        ...
        df_cbm = pd.DataFrame({'period':pi["timestep"] * 0.1, 
                               ...
                               'total_emission':fi[fluxes].sum(axis=1),
                               'gross_growth':fi[gross_growth].sum(axis=1),
                               'product_flux':fi[product_flux].sum(axis=1)}).groupby('period').sum().iloc[10::10, :].reset_index()
        df_cbm['period'] = (df_cbm['period']).astype(int)
        
    df_cbm['eco_stock_change']=df_cbm['eco_stock'].diff()
    # df_cbm.at[0,'stock_change'] = initial_ec_stock - df_cbm.at[0,'eco_stock']
    df_cbm.at[0,'eco_stock_change'] = 0.
    # df_cbm['net_emission_eg']=df_cbm['total_emission']-df_cbm['gross_growth'] #Anuual
    df_cbm['net_emission_sp']=-df_cbm['eco_stock_change']-df_cbm['product_flux'] #Periodic
    # df_cbm['product_change']=df_cbm['product_stock'].diff()
    # df_cbm.at[0,'product_change'] = 0.
    df_cbm.at[0,'net_emission_sp'] = 0.
    ...
    df_ws3 = pd.DataFrame({'period':fm.periods,
                           ...
                           'total_emission':[sum(fm.inventory(period, pool) for pool in ['total_emissions']) for period in fm.periods],
                           'product_flux':[sum(fm.compile_product(period, pool, acode='harvest') * 0.85 * 460 * 0.5 * 0.001 for pool in ['totvol']) for period in fm.periods]})

    df_ws3['eco_stock_change']=df_ws3['eco_stock'].diff()
    df_ws3.at[0,'eco_stock_change'] = 0.
    df_ws3['net_emission_sp']=-df_ws3['eco_stock_change']-df_ws3['product_flux']
    df_ws3.at[0,'net_emission_sp'] = 0.
    # df_ws3['product_change']=df_ws3['product_stock'].diff()
    # df_ws3.at[0,'product_change'] = 0.
    ...
    # Optional: Set the y-axis limits if necessary
    ...
    # Plotting the data
    plt.plot(df_cbm['period'], df_cbm['eco_stock_change'], label='cbm ecosystem flux')
    plt.plot(df_ws3['period'], df_ws3['eco_stock_change'], label='ws3 ecosystem flux')
...
#     fix, ax = plt.subplots(5, 1, figsize=(10, 12), sharex=True)
#     ax[0].plot(df_cbm['period'], df_cbm['eco_stock'], label='cbm ecosystem stock')
#     ax[0].plot(df_ws3['period'], df_ws3['eco_stock'], label='ws3 ecosystem stock')
#     ax[0].plot(df_cbm['period'], df_cbm['dom_stock'], label='cbm DOM stock')
#     ax[0].plot(df_ws3['period'], df_ws3['dom_stock'], label='ws3 DOM stock')
#     ax[0].plot(df_cbm['period'], df_cbm['biomass_stock'], label='cbm biomass stock')
#     ax[0].plot(df_ws3['period'], df_ws3['biomass_stock'], label='ws3 biomass stock')
#     # ax[0].plot(df_cbm['period'], df_cbm['CO2'], label='cbm CO2 pool')
#     # ax[0].plot(df_ws3['period'], df_ws3['CO2'], label='ws3 CO2 pool')
#     ax[1].plot(df_cbm['period'], df_cbm['total_emission'], label='cbm total emissions')
#     ax[1].plot(df_ws3['period'], df_ws3['total_emission'], label='ws3 total emissions')
#     ax[2].plot(df_cbm['period'], df_cbm['stock_change'], label='cbm stock change')
#     ax[2].plot(df_ws3['period'], df_ws3['stock_change'], label='ws3 stock change')
#     # ax[2].plot(df_ws3['period'], df_ws3['net_fluxes'], label='ws3 net fluxes')
#     ax[3].plot(df_cbm['period'], df_cbm['product_flux'], label='cbm product flux')
#     ax[3].plot(df_ws3['period'], df_ws3['product_flux'], label='ws3 product flux')
#     # ax[4].plot(df_cbm['period'], df_cbm['net_emission_eg'], label='cbm net emission (e-g)')
#     ax[4].plot(df_cbm['period'], df_cbm['net_emission_sp'], label='cbm net emission (s-p)')
#     ax[4].plot(df_ws3['period'], df_ws3['net_emission_sp'], label='ws3 net emission (s-p)')    
...
#     ax[4].legend(bbox_to_anchor=(1.01, 1),loc='upper left', borderaxespad=0.)
#     ax[0].set_ylim(None, None)
#     ax[1].set_ylim(None, None)
#     ax[2].set_ylim(None, None)
#     ax[3].set_ylim(None, None)
#     ax[4].set_ylim(None, None)
    
#     df_cbm = df_cbm.apply(lambda x: x.round(3) if x.dtype.kind in 'fc' else x)
#     df_ws3 = df_ws3.apply(lambda x: x.round(3) if x.dtype.kind in 'fc' else x)
